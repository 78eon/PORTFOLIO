      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white">
        <header className="border-b border-[#222] px-6 py-4">
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {unread > 0 && (
              <span className="bg-[#00ff41] text-black text-xs font-bold px-2 py-0.5 rounded-full">
                {unread} new
              </span>
            )}
          </div>
          ...
          {messages.length === 0 ? (
            ...
            <div className="space-y-4">
              {messages.map(msg => (
                <div
                  key={msg.id}
                  className={`border rounded-lg p-5 ${msg.read ? 'border-[#1a1a1a] bg-[#0c0c0c]' : 'border-[#00ff41]/20 bg-[#0d0d0d]'}`}
                >
