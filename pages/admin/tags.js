...
export async function getServerSideProps() {
  ...
  return { props: { initialTags: rows } }
}

export default function TagsAdmin({ initialTags }) {
  ...
  async function deleteTag(id, tagName) {
    if (!confirm(`Delete tag "${tagName}"? It will be removed from all writeups.`)) return
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white">
        ...
        <main className="max-w-2xl mx-auto px-6 py-8">
            ...
            <form onSubmit={createTag} className="flex gap-3 items-end flex-wrap">
              <div className="flex-1 min-w-40">
                ...
                <input
                  ...
                  className="w-full bg-[#0a0a0a] border border-[#333] rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-[#00ff41] transition-colors"
                  placeholder="e.g. SQLi, XSS, OWASP..."
                  required
                />
              </div>
              <div>
              ...
              <button
                type="submit"
                disabled={saving}
                className="bg-[#00ff41] text-black font-bold text-sm px-5 py-2 rounded hover:bg-[#00cc33] disabled:opacity-40 transition-colors"
              >
          ...
          {/* List */}
          {tags.length === 0 ? (
            <p className="text-[#444] font-mono text-sm text-center py-12">No tags yet. Create your first one above.</p>
          ) : (
                  ...
                  {editId === tag.id ? (
                    <>
                      <input
                        ...
                        className="w-8 h-8 rounded cursor-pointer border border-[#333] bg-transparent flex-shrink-0"
                      />
                      <input
                        type="text"
                        value={editData.name}
                        onChange={e => setEditData(d => ({ ...d, name: e.target.value }))}
                        className="flex-1 bg-[#0a0a0a] border border-[#333] rounded px-3 py-1.5 text-white text-sm focus:outline-none focus:border-[#00ff41]"
                      />
                    ...
                    <>
                      ...
                      <button
                        onClick={() => { setEditId(tag.id); setEditData({ name: tag.name, color: tag.color }) }}
